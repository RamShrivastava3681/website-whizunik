  ...
  return (
    <section ref={containerRef} className="py-48 bg-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-8 relative z-10">
        ...
        >
          <span className="inline-block px-4 py-1.5 bg-primary/10 rounded-full text-primary font-bold uppercase tracking-[0.2em] text-[9px] mb-6 shadow-sm">Operational Protocol</span>
          <h2 className="serif-headline text-5xl md:text-6xl font-bold text-black tracking-tight">
            How We <span className="blue-gradient-text italic font-light">Work.</span>
          </h2>
        </motion.div>
          ...
          <div className="space-y-24">
            {steps.map((step, i) => (
              <motion.div 
                ...
                className={`relative flex flex-col md:flex-row items-center gap-12 md:gap-20 ${i % 2 !== 0 ? 'md:flex-row-reverse' : ''}`}
              >
                {/* Step Content */}
                <div className={`flex-1 text-center ${i % 2 !== 0 ? 'md:text-left' : 'md:text-right'}`}>
                   <span className="font-headline text-6xl text-black/5 font-bold mb-2 block tabular-nums">
                     {step.num}
                   </span>
                   <h4 className="text-2xl font-headline text-black mb-4 font-bold">{step.title}</h4>
                   <p className="text-black/80 text-lg font-medium mb-3">{step.desc}</p>
                   <p className="text-black/40 text-xs leading-relaxed max-w-xs mx-auto md:mx-0">
                     {step.detail}
                   </p>
                </div>
